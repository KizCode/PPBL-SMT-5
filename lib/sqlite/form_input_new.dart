...
class _FormInputSahamState extends State<FormInputSaham> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Form Input Saham"),
      ),
          ...
          child: Column(
            children: [
              TextFormField(
                ...
                decoration: const InputDecoration(labelText: 'Ticker (kode saham)', border: OutlineInputBorder()),
                validator: (v) => (v == null || v.trim().isEmpty) ? 'Ticker wajib diisi' : null,
              ),
              const SizedBox(height: 10),
              TextFormField(
                controller: _openController,
                decoration: const InputDecoration(labelText: 'Open (harga pembukaan)', border: OutlineInputBorder()),
                keyboardType: TextInputType.number,
                validator: (v) => (v == null || v.trim().isEmpty) ? 'Open wajib diisi' : null,
              ),
              const SizedBox(height: 10),
              TextFormField(
                controller: _highController,
                decoration: const InputDecoration(labelText: 'High (harga tertinggi)', border: OutlineInputBorder()),
                keyboardType: TextInputType.number,
                validator: (v) => (v == null || v.trim().isEmpty) ? 'High wajib diisi' : null,
              ),
              const SizedBox(height: 10),
              TextFormField(
                controller: _lastController,
                decoration: const InputDecoration(labelText: 'Last (harga terbaru)', border: OutlineInputBorder()),
                keyboardType: TextInputType.number,
                validator: (v) => (v == null || v.trim().isEmpty) ? 'Last wajib diisi' : null,
              ),
              const SizedBox(height: 10),
              TextFormField(
                controller: _changeController,
                decoration: const InputDecoration(labelText: 'Change (%)', border: OutlineInputBorder()),
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                validator: (v) => (v == null || v.trim().isEmpty) ? 'Change wajib diisi' : null,
              ),
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => const ReadSaham()),
                      );
                    },
